...
public class RestControllerTest {
	...
	public ResponseEntity<?> test3(@PathVariable Integer id) {
		// 먼저 URI 객체 만들기
		URI uri = UriComponentsBuilder.fromUriString("https://jsonplaceholder.typicode.com").path("/posts")
				.path("/" + id).encode().build().toUri();
		...
		// 바디구성
		MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
		params.add("title", "수정제목");
		params.add("body", "수정내용");
		params.add("userID", "2");
